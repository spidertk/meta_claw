...
- [x] 当前进度已经记录到进度日志
  证据：`claude-progress.md` 已新增 Session 054，记录流式工具调用回调未触发问题的修复、测试与验证结果
- [x] 功能状态真实反映了 passing 和未验证的边界
  证据：`feature_list.json` 的 `spi-002` 已补充 `onToolCall` 修复记录；主设计文档中的风险与不足点未改变，仍按原状态跟踪
- [x] 没有任何半成品步骤处于未记录状态
  证据：`LlmClientManager` 修改、文档与状态文件更新均已完成
- [x] 下一轮会话无需人工修复即可继续
