 ...
 * EventBus 消息流闭环集成测试
 * <p>
 * 验证 Meta-Claw 系统中从用户消息接收到 Vessel 回复就绪的完整异步事件流：
 * 1. Gateway 发布 UserMessageReceived 事件
 * 2. AgentLoop 订阅并处理用户消息
 * 3. AgentLoop 调用 VesselRuntime 生成回复
 * 4. AgentLoop 发布 VesselResponseReady 事件
...
class MessageFlowIntegrationTest {
    ...
    /** Vessel 管理器，提供 Vessel 配置与运行时实例 */
    private VesselManager vesselManager;

    /** Agent 事件循环处理器，订阅用户消息并调度 Vessel */
    private AgentLoop agentLoop;

    /** 异步等待锁，用于在异步事件流中同步等待结果 */
    private CountDownLatch latch;

    /** 捕获到的 VesselResponseReady 事件，用于断言验证 */
    private VesselResponseReady capturedEvent;

    /**
     * 每个测试方法执行前的初始化操作
     * <p>
     * 构建完整的组件链路：EventBus → Gateway + AgentLoop，
     * 并用 Mockito 模拟 VesselManager 和 VesselRuntime，
    ...
    void setUp() {
        ...
        // 使用 Mockito 模拟 VesselManager，控制可用 Vessel 列表
        vesselManager = mock(VesselManager.class);
        VesselConfig testConfig = new VesselConfig();
        testConfig.setId("test-vessel");
        testConfig.setName("测试 Vessel");
        when(vesselManager.listAvailableVessels()).thenReturn(Collections.singletonList(testConfig));

        // 使用 Mockito 模拟 VesselRuntime，使其返回预设的测试回复
        VesselRuntime mockRuntime = mock(VesselRuntime.class);
        when(mockRuntime.chat(anyString())).thenReturn(new Reply(ReplyType.TEXT, "集成测试回复内容"));
        when(vesselManager.getRuntime("test-vessel")).thenReturn(mockRuntime);

        // 初始化 AgentLoop，它会自动注册为 EventBus 订阅者
        agentLoop = new AgentLoop(eventBus, vesselManager);

        // 初始化异步等待锁与事件捕获变量
        latch = new CountDownLatch(1);
        capturedEvent = null;

        // 注册测试订阅者，用于捕获 VesselResponseReady 事件并释放等待锁
     ...
     * 期望 AgentLoop 调度 Vessel 处理后，通过 EventBus 发布 VesselResponseReady 事件，
    ...
    void testMessageFlowLoop() throws InterruptedException {
        ...
        // 断言一：必须在超时前收到 VesselResponseReady 事件
        assertTrue(received, "应在 5 秒内收到 VesselResponseReady 事件");
        ...
        assertEquals(channelType, capturedEvent.getChannelType(),
                "VesselResponseReady 中的 channelType 应与原始消息一致");
        ...
        assertEquals("集成测试回复内容", capturedEvent.getReply().getContent(),
                "回复内容应与 VesselRuntime 生成的回复一致");
    }

    /**
     * 测试：验证 EventBus 的基础发布/订阅机制
     * <p>
     * 场景：直接发布 VesselResponseReady 事件，不经过 AgentLoop，
    ...
    void testEventBusPubSub() throws InterruptedException {
        ...
        // 断言：必须在超时前收到事件
        assertTrue(received, "应在 3 秒内收到 VesselResponseReady 事件");
        assertNotNull(capturedEvent, "捕获的事件不应为 null");
        assertEquals(channelType, capturedEvent.getChannelType(),
                "channelType 应正确传递");
    ...
    /**
     * 测试：当系统中没有可用 Vessel 时，AgentLoop 应发布 ERROR 类型回复
     * <p>
     * 场景：使用独立的事件总线，模拟空的 VesselManager，发布用户消息后，
     ...
     */
    @Test
    void testMessageFlowWithNoVessel() throws InterruptedException {
        // 创建独立的事件总线，避免与 setUp 中的 AgentLoop 产生干扰
        EventBusWrapper isolatedEventBus = new EventBusWrapper();

        // 初始化一个空的 VesselManager（不返回任何可用 Vessel）
        VesselManager emptyManager = mock(VesselManager.class);
        when(emptyManager.listAvailableVessels()).thenReturn(Collections.emptyList());

        // 创建 AgentLoop，绑定到空的 VesselManager，自动注册为订阅者
        AgentLoop emptyAgentLoop = new AgentLoop(isolatedEventBus, emptyManager);

        // 初始化独立的等待锁与事件捕获变量
        CountDownLatch noVesselLatch = new CountDownLatch(1);
        VesselResponseReady[] noVesselCaptured = new VesselResponseReady[1];

        // 注册测试订阅者到独立事件总线
        isolatedEventBus.register(new Object() {
            @Subscribe
            public void onVesselResponseReady(VesselResponseReady event) {
                noVesselCaptured[0] = event;
                noVesselLatch.countDown();
            }
        });

        // 发布用户消息事件
        Context context = new Context(ContextType.TEXT, "没有 Vessel 时的测试消息");
        isolatedEventBus.post(new UserMessageReceived(context, "session-no-vessel", "douyin"));

        // 等待异步处理
        boolean received = noVesselLatch.await(3, TimeUnit.SECONDS);

        // 断言：即使没有 Vessel，也应在超时前收到 ERROR 回复事件
        assertTrue(received, "即使没有 Vessel，也应在 3 秒内收到回复事件");
        assertNotNull(noVesselCaptured[0], "捕获的事件不应为 null");
        assertEquals("douyin", noVesselCaptured[0].getChannelType(),
                "channelType 应正确传递");
        assertEquals(ReplyType.ERROR, noVesselCaptured[0].getReply().getType(),
                "当没有可用 Vessel 时，回复类型应为 ERROR");
    }

    /**
     * 内部测试订阅者类
     * <p>
     * 使用 Guava EventBus 的 @Subscribe 注解订阅 VesselResponseReady 事件，
    ...
    private class TestResponseSubscriber {
        ...
        /**
         * 订阅 VesselResponseReady 事件的处理方法
         *
         * @param event Vessel 回复就绪领域事件
         */
        @Subscribe
        public void onVesselResponseReady(VesselResponseReady event) {
            capturedEvent = event;
