...
当前 Java 骨架中的单批上限只能被解释为 `scan batch size`，不能解释为最终分析上限。超大来源必须通过多批扫描逐步补全，并在 snapshot 中记录 `scanStatus`、`unitLimit`、`includedUnitCount`、`scanBatchCount` 等覆盖元数据。
...
记录来源稳定身份、来源元数据、当前处理状态，以及指向当前最新快照的 `latest_snapshot_id` 派生指针。

当前代码基线（2026-04-25）中，Java domain 使用 camelCase 字段 `latestSnapshotId` 表达同一语义；语言无关 contract 与 JSON example 使用 snake_case 字段 `latest_snapshot_id` 对齐。

#### 8.2.2 `snapshot_store`

记录按 `source_id` 关联的不可变版本快照及其结构信息；它保存历史事实，不承担来源当前状态主表职责。
...
`scanStatus=partial` 或 `scanStatus=truncated` 的 snapshot 只能进入局部 artifact、候选知识或 review 队列；只有 `scanStatus=complete` 才允许发布全局结论。
