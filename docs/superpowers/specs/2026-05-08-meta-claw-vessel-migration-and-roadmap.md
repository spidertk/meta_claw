...
| 当前名称 | 目标名称 | 所在模块 | 影响范围 |
|---------|---------|---------|---------|
| `ExpertConfig` | 删除，复用 `meta-claw-vessel.VesselConfig` | meta-claw-core | 被 ExpertManager、ExpertRuntime、AppConfig 引用 |
| `ExpertManager` | `VesselManager` | meta-claw-core | 被 AgentLoop、AppConfig 引用 |
| `ExpertRuntime` | `VesselRuntime` | meta-claw-core | 被 AgentLoop、AppConfig 引用 |
| `ExpertResponseReady` | `VesselResponseReady` | meta-claw-core | 被 Gateway、AgentLoop 引用 |
| `SessionManagerTest` 中的 "Expert" 字符串 | 改为 "Vessel" | meta-claw-core test | 测试用例描述 |

### 1.2 必须替换的方法/变量/注释

| 文件 | 替换内容 |
|-----|---------|
| `ExpertManager.java` | 类名、构造参数 `expertsDir` → `vesselsDir`、`DEFAULT_EXPERTS_DIR` → `DEFAULT_VESSELS_DIR`、所有方法名中的 `expert` → `vessel`、`Expert` → `Vessel` |
| `ExpertRuntime.java` | 类名、构造参数中的 `ExpertConfig` → `VesselConfig` |
| `AgentLoop.java` | 引用 `ExpertManager` → `VesselManager`、`ExpertRuntime` → `VesselRuntime` |
| `AppConfig.java` | `expertManager()` → `vesselManager()`、`initializeRuntimes()` 中的 `ExpertConfig` → `VesselConfig`、`ExpertRuntime` → `VesselRuntime`、`ExpertManager` → `VesselManager` |
| `Gateway.java` | `ExpertResponseReady` → `VesselResponseReady` |
| `ChatCommand.java` | `ExpertConfig` → `VesselConfig`、`ExpertRuntime` → `VesselRuntime` |
| `MessageFlowIntegrationTest.java` | `ExpertResponseReady` → `VesselResponseReady` |
| `MetaClawApplication.java` | 如有 expert 引用则替换 |

### 1.3 配置与资源文件

| 文件 | 处理方案 |
|-----|---------|
| `meta-claw-bootstrap/src/main/resources/experts/` | 目录重命名为 `vessels/`，配置格式从 `expert.yaml` 迁移为 `vessel.md` |
| `AppConfig.java` 中的 `${meta.claw.experts.dir}` | 改为 `${meta.claw.vessels.dir}`，默认值改为 `vessels` |
...
- `VesselConfigLoader` 只解析 `vessel.md` 的 YAML frontmatter，**没有解析 Markdown body**（Identity、Soul、Domain Knowledge 等 section）
- 没有使用 `vessel.tmpl.md` 作为创建新 Vessel 的模板
- `VesselConfig` 只包含 frontmatter 字段，缺少 `role`、`autoServe` 等字段
...
- 解析 YAML frontmatter 后，继续解析 `---` 之后的 Markdown body
- 按 `## SectionName` 分割，提取各 section 内容
- 新增 `loadTemplate()` 方法加载 `vessel.tmpl.md`

#### C. 新增 VesselTemplate（参考 expert_cli/cli.py 的 create 命令）

```java
public class VesselTemplate {
    /**
     * 基于 vessel.tmpl.md 创建新 Vessel 目录结构
     */
...
```
vessels/{name}/
├── vessel.md          # 从 vessel.tmpl.md 渲染
├── config.yaml        # 可选配置（provider、model、max_history、temperature）
├── skills/
├── knowledge/
...
| 命令 | 功能 | 优先级 |
|-----|------|--------|
| `meta-cli init` | 初始化 `~/.meta-claw/`：config.yaml + vessels/default/ + skills/ | P0 |
| `meta-cli create <name>` | 基于 vessel.tmpl.md 创建新 Vessel | P0 |
| `meta-cli chat <name>` | 本地交互式对话（单消息 / REPL 模式） | P0 |
| `meta-cli list` | 列出所有 Vessel | P0 |
| `meta-cli delete <name>` | 删除 Vessel | P1 |

### 3.2 Phase 2-3 命令规划

| 命令 | 功能 | 优先级 |
|-----|------|--------|
| `meta-cli config get/set/list` | 操作 `~/.meta-claw/config.yaml` | P1 |
| `meta-cli serve [names...]` | 前台启动 WebSocket 服务 | P1 |
| `meta-cli start [names...]` | 后台 daemon 启动（alias: serve --daemon） | P1 |
| `meta-cli stop` | 停止后台服务 | P1 |
| `meta-cli restart [names...]` | 重启后台服务 | P1 |
| `meta-cli status` | 查看运行状态 | P2 |
| `meta-cli logs` | 查看日志 | P2 |
...
| 优点 | 吸收方案 | 优先级 |
|-----|---------|--------|
| 多 Expert 隔离架构 | Vessel 目录完全隔离（已有），确保 conversations/ 和 preferences/ 也隔离 | P0 |
| 工具自动发现 | Phase 3 实现 `ToolRegistry` 自动扫描 `@Tool` 注解 | P2 |
| Skill 即提示词 | Phase 4 实现 Skill 为 Markdown 文件，注入系统提示 | P3 |
| EventBus 解耦（已有） | 保持现有设计，扩展更多事件类型 | P1 |
| 分层日志 | 按模块拆分日志（cli/gateway/vessel/agent_loop/tools） | P2 |
| 多 Provider 统一接入（已有） | 保持 `SpiLlmClient` SPI 设计 | — |
| MCP 协议集成 | Phase 4 实现 `meta-claw-mcp` | P3 |
| Cron 定时任务 | Phase 5 考虑添加定时任务支持 | P5 |

### 4.2 来自 CowAgent

| 优点 | 吸收方案 | 优先级 |
|-----|---------|--------|
| 多渠道并发架构 | `ChannelManager` 用独立线程管理多通道，支持热插拔 | P2 |
| Bridge 桥接层 | 新增 `Bridge` 单例，统一调度 chat/语音/翻译 | P3 |
| 插件事件系统 | 四阶段事件（ON_RECEIVE / ON_HANDLE / ON_DECORATE / ON_SEND）+ 优先级 | P3 |
| 会话隔离（已有改进） | 每个 session 独立队列 + 信号量，避免消息串扰 | P1 |
| 工具阶段 | `ToolStage`：PRE_PROCESS（Agent 选择）/ POST_PROCESS（答案后自动执行） | P3 |
| 上下文智能压缩 | Token 超出时自动压缩历史，保留核心信息 | P2 |
| 缺依赖优雅降级 | 工具加载失败时记录警告而非崩溃 | P2 |
| 条件工具加载 | 依赖 API Key 的工具，缺配置时静默禁用 | P2 |

### 4.3 来自 Claude Code

| 优点 | 吸收方案 | 优先级 |
|-----|---------|--------|
| 权限系统 | `PermissionMode` + 规则引擎（alwaysAllow/alwaysDeny/alwaysAsk） | P3 |
| 查询引擎 QueryEngine | `QueryEngine` 管理消息历史、Token 限制、费用追踪、后台任务 | P2 |
| 工具 UI 渲染 | 工具调用时显示进度/结果（终端 UI） | P3 |
| MCP 完整客户端（已有规划） | stdio/sse/http/ws 多传输、OAuth、资源读取 | P3 |
| 状态管理 | 自定义轻量 Store（消息历史、权限上下文、工具状态） | P2 |
| 对话压缩 compact | Token 超限时自动压缩对话历史 | P2 |
| Bash 安全解析链 | AST → 命令分类 → 路径约束 → 权限规则 | P4 |
...
- [x] 检查并更新所有 Java 注释中的 "Expert" → "Vessel"
...
- [x] 增强 `VesselTemplate`：从 classpath 加载 `templates/vessel.tmpl.md`，支持基于模板创建任意 Vessel
...
- [ ] 对话历史追加到 `JsonlConversationStore`（Batch 4 完善）
...
├── VesselTemplate.java          # 基于模板创建新 Vessel
